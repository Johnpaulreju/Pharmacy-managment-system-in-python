...
class Emppage:
    ...
    def __init__(self, root,username):
        ...
        self.reportmenu.add_command(label="Copy all")
        self.reportmenu.add_command(label="Clear all")
        self.reportmenu.add_command(label="Undo")
        ...
        self.heading = Label(self.root, text=self.txt, font=(
            "times new roman", 30, "bold"), background="#283e4a", fg="blue", bd=9, relief=RIDGE)
        ...
        down_buttonframe.place(x=0, y=512, width=1535, height=60)
            ...
            ###### all buttons ######
        add_button = Button(down_buttonframe, text="Store Informtion", command=self.addmedicine, font=(
...
# print(len(row_01))
        # # for i in range(0,len(row_01)): 
        # self.ref_combo["values"] = ('Select',"ab")
    ...
    def balan(self):
        ...
        if (ttp<0):
            messagebox.showwarning("Bill Error","Amount is not paid fully")
        self.balance.set(ttp)
        messagebox.showwarning("DATA STORING NOTIFICATION","Store The Information After The Notification")
        ...
        if selected == "Select Options":
            ...
            if self.search_by.get()=="Mobile No.":
                ...
                if len(row)!=0:
                    ...
                    messagebox.showwarning("ERROR","NO DATA IN THE TABLE")
